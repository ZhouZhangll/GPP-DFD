...
import numpy as np
from models.model_bert import GDLayerNorm
from models.indicator_vector_op import *
from transformers.file_utils import hf_bucket_url, cached_path

logger = logging.getLogger(__name__)

BertLayerNorm = GDLayerNorm


class GDRobertaForSequenceClassification(RobertaForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
        self.roberta = GDRobertaModel(config)
...
class GDRobertaBertForMaskedLM(RobertaForMaskedLM):
    def __init__(self, config):
        super().__init__(config)
        self.roberta = GDRobertaModel(config)
...
class GDRobertaEmbeddings(RobertaEmbeddings):
...
class GDRobertaModel(RobertaModel):
    def __init__(self, config):
        super().__init__(config)
        self.encoder = GDRobertaEncoder(config)
        self.embeddings = GDRobertaEmbeddings(config)
    ...
    def forward(
        ...
        return GDBaseModelOutputWithPooling(
...
class GDRobertaEncoder(RobertaEncoder):
    def __init__(self, config):
        super().__init__(config)
        self.layer = nn.ModuleList([GDRobertaLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                layer_outputs = layer_module(
                    ...
                    headlayer=headlayer if headlayer is not None else None,
                    inference=inference,
                    hidden=hidden
                )
        ...
        return GDBaseModelOutput(
            last_hidden_state=hidden_states, hidden_states=all_hidden_states, attentions=all_attentions,
            attention_layers=all_attention_outputs
        )


class GDRobertaLayer(RobertaLayer):
    def __init__(self, config):
        super().__init__(config)
        self.attention = GDRobertaAttention(config)
        self.output = GDRobertaOutput(config)
        self.config = config
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        layer_output = self.output(intermediate_output, attention_output, self.intlayer, self.hidden,
                                   inference=self.inference)
        return layer_output


class GDRobertaAttention(RobertaAttention):
    def __init__(self, config):
        super().__init__(config)
        self.self = GDRobertaSelfAttention(config)
        self.output = GDRobertaSelfOutput(config)
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, headlayer=headlayer, hidden=hidden,
                                       inference=inference)
        outputs = (attention_output,) + self_outputs[1:]  # add attentions if we output them
        return outputs


class GDRobertaSelfAttention(RobertaSelfAttention):
    def __init__(self, config):
        super().__init__(config)
...
class GDRobertaSelfOutput(RobertaSelfOutput):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, hidden_states, input_tensor, headlayer=None, hidden=None, inference=False):
        ...
        if not inference and hidden_states.sum().eq(0).item():
            ...
            hidden_states = self.LayerNorm(hidden_states + input_tensor, hidden)
...
class GDRobertaOutput(RobertaOutput):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, hidden_states, input_tensor, intlayer, hidden=None, inference=False):
        hidden_states = self.dense(hidden_states)
...
@dataclass
class GDQuestionAnsweringModelOutput(ModelOutput):
...
@dataclass
class GDBaseModelOutputWithPooling(ModelOutput):
...
@dataclass
class GDBaseModelOutput(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    attention_layers: Optional[Tuple[torch.FloatTensor]] = None
